...
void DumpMethodCFG(const ClassAccessor::Method &method, std::ostream &os) {
    ...
    std::set<uint32_t> dex_pc_is_branch_target;
    {
        ...
        for (const DexInstructionPcPair &pair : accessor) {
            ...
            if (inst->IsBranch()) {
                dex_pc_is_branch_target.insert(pair.DexPc() +
                                               inst->GetTargetOffset());
            } else if (inst->IsSwitch()) {
                const uint16_t *insns =
                    reinterpret_cast<const uint16_t *>(inst);
                int32_t switch_offset =
                    insns[1] | (static_cast<int32_t>(insns[2]) << 16);
                ...
                for (uint32_t targ = 0; targ < switch_count; targ++) {
                    int32_t offset =
                        static_cast<int32_t>(
                            switch_insns[targets_offset + targ * 2]) |
                        static_cast<int32_t>(
                            switch_insns[targets_offset + targ * 2 + 1] << 16);
    ...
    std::map<uint32_t, uint32_t>
        dex_pc_to_node_id; // This only has entries for block starts.
    std::map<uint32_t, uint32_t>
        dex_pc_to_incl_id; // This has entries for all dex pcs.

    {
        ...
        for (const DexInstructionPcPair &pair : accessor) {
            ...
            if (dex_pc == 0 ||
                (dex_pc_is_branch_target.find(dex_pc) !=
                 dex_pc_is_branch_target.end()) ||
                force_new_block) {
            ...
            dex_pc_to_incl_id.insert(
                std::make_pair(dex_pc, dex_pc_to_node_id.size() - 1));
            ...
            size_t cur_start = 0; // It's OK to start at zero, instruction dumps
                                  // don't start with chars we need to escape.
            while (cur_start != std::string::npos) {
                size_t next_escape =
                    inst_str.find_first_of("\"{}<>", cur_start + 1);
                if (next_escape == std::string::npos) {
                    os << inst_str.substr(cur_start,
                                          inst_str.size() - cur_start);
                    break;
                } else {
            ...
            // Force a new block for some fall-throughs and some instructions
            // that terminate the "local" control flow.
            force_new_block =
                pair.Inst().IsSwitch() || pair.Inst().IsBasicBlockEnd();
        }
        // Close last node.
    ...
    // Create edges between them.
    {
        ...
        // These blocks (given by the first dex pc) need exception per dex-pc
        // handling in a second pass. In the first pass we try and see whether
        // we can use a common set of edges.
...

        {
            ...
            for (const DexInstructionPcPair &pair : accessor) {
                ...
                const uint32_t dex_pc = pair.DexPc();
                {
                    ...
                    if (it != dex_pc_to_node_id.end()) {
                        if (!exception_targets.empty()) {
                            // It seems the last block had common exception
                            // handlers. Add the exception edges now.
                            uint32_t node_id =
                                dex_pc_to_node_id.find(block_start_dex_pc)
                                    ->second;
                            for (uint32_t handler_pc : exception_targets) {
                                auto node_id_it =
                                    dex_pc_to_incl_id.find(handler_pc);
                        ...
                        // Seems to be a fall-through, connect to last_node_id.
                        // May be spurious edges for things like switch data.
                        uint32_t old_last = last_node_id;
                        last_node_id = it->second;
                        if (old_last != std::numeric_limits<uint32_t>::max()) {
                            regular_edges << "  node" << old_last << ":p"
                                          << old_dex_pc << " -> node"
                                          << last_node_id << ":p" << dex_pc
                                          << ";\n";
                        }
                    }
                ...
                if (inst->IsBranch()) {
                    ...
                    if (target_it != dex_pc_to_node_id.end()) {
                        taken_edges << "  node" << last_node_id << ":p"
                                    << dex_pc << " -> node" << target_it->second
                                    << ":p" << (dex_pc + offset) << ";\n";
                    }
                    if (!conditional) {
                        // No fall-through.
                    ...
                    for (uint32_t targ = 0; targ < switch_count; targ++) {
                        int32_t offset =
                            static_cast<int32_t>(
                                switch_insns[targets_offset + targ * 2]) |
                            static_cast<int32_t>(
                                switch_insns[targets_offset + targ * 2 + 1]
                                << 16);
                ...
                // Exception edges. If this is not the first instruction in the
                // block
                if (block_start_dex_pc != dex_pc) {
                    std::set<uint32_t> current_handler_pcs;
                    CatchHandlerIterator catch_it(accessor, dex_pc);
                    for (; catch_it.HasNext(); catch_it.Next()) {
                        current_handler_pcs.insert(
                            catch_it.GetHandlerAddress());
                    }
                    if (current_handler_pcs != exception_targets) {
                        exception_targets.clear(); // Clear so we don't do
                                                   // something at the end.
                        blocks_with_detailed_exceptions.insert(
                            block_start_dex_pc);
                    }
                }
            ...
            if (!exception_targets.empty()) {
                // It seems the last block had common exception handlers. Add
                // the exception edges now.
                uint32_t node_id =
                    dex_pc_to_node_id.find(block_start_dex_pc)->second;
                for (uint32_t handler_pc : exception_targets) {
                    auto node_id_it = dex_pc_to_incl_id.find(handler_pc);
                    if (node_id_it != dex_pc_to_incl_id.end()) {
                        exception_edges << "  node" << node_id << " -> node"
                                        << node_id_it->second << ":p"
                                        << handler_pc << ";\n";
                    }
                }
        ...
        for (uint32_t dex_pc : blocks_with_detailed_exceptions) {
            ...
            while (true) {
                    ...
                    for (; catch_it.HasNext(); catch_it.Next()) {
                        ...
                        if (it == handled_targets.end()) {
                            auto node_id_it =
                                dex_pc_to_incl_id.find(handler_pc);
                            if (node_id_it != dex_pc_to_incl_id.end()) {
                                exception_edges << "  node" << this_node_id
                                                << ":p" << dex_pc << " -> node"
                                                << node_id_it->second << ":p"
                                                << handler_pc << ";\n";
                            }
                ...
                // Loop update. Have a break-out if the next instruction is a
                // branch target and thus in another block.
