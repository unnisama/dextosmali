...
enum class LabelEnum {
    ...
    SPARSE_SWITCH_LABEL,
    PACKED_SWITCH_LABEL,
    DATA_SPARSE_SWITCH_LABEL,
    DATA_PACKED_SWITCH_LABEL,
    ARRAY_DATA_LABEL
};
...
static inline void printcatch(u4 start, u4 end,
                              std::pair<u4, std::string> catch_) {
    if (catch_.second == "<all>") {
        fprintf(gOutFile,
                "    .catchall {:try_start_%x .. "
                ":try_end_%x} :catchall_%x\n\n",
                (u2)start, (u2)end, (u2)catch_.first);
    } else {
        fprintf(gOutFile,
                "    .catch %s {:try_start_%x .. "
                ":try_end_%x} :catch_%x\n\n",
                catch_.second.c_str(), (u2)start, (u2)end, (u2)catch_.first);
    }
}

inline char getporv(u4 reg, u4 pnum, u4 *v) {
    if (reg >= pnum) {
        *v = reg - pnum;
        return 'p';
    } else {
...
static void printLabel(LabelEnum ltype, u4 insnIdx) {
    std::string sp = "    ";
    switch (ltype) {
        case LabelEnum::IF_LABEL:
            fprintf(gOutFile, "%s:cond_%x\n", sp.c_str(), (u2)insnIdx);
            break;
        case LabelEnum::GOTO_LABEL:
            fprintf(gOutFile, "%s:goto_%x\n", sp.c_str(), (u2)insnIdx);
            break;
        case LabelEnum::TRY_START_LABEL:
            fprintf(gOutFile, "%s:try_start_%x\n", sp.c_str(), (u2)insnIdx);
            break;
        case LabelEnum::TRY_END_LABEL:
            fprintf(gOutFile, "%s:try_end_%x\n", sp.c_str(), (u2)insnIdx);
            break;
        case LabelEnum::CATCH_LABEL:
            fprintf(gOutFile, "%s:catch_%x\n", sp.c_str(), (u2)insnIdx);
            break;
        case LabelEnum::CATCHALL_LABEL:
            fprintf(gOutFile, "%s:catchall_%x\n", sp.c_str(), (u2)insnIdx);
            break;
        case LabelEnum::PACKED_SWITCH_LABEL:
            fprintf(gOutFile, "%s:pswitch_%x\n", sp.c_str(), (u2)insnIdx);
            break;
        case LabelEnum::SPARSE_SWITCH_LABEL:
            fprintf(gOutFile, "%s:sswitch_%x\n", sp.c_str(), (u2)insnIdx);
            break;
        case LabelEnum::DATA_PACKED_SWITCH_LABEL:
            fprintf(gOutFile, "%s:pswitch_data_%x\n", sp.c_str(), (u2)insnIdx);
            break;
        case LabelEnum::DATA_SPARSE_SWITCH_LABEL:
            fprintf(gOutFile, "%s:sswitch_data_%x\n", sp.c_str(), (u2)insnIdx);
            break;
        case LabelEnum::ARRAY_DATA_LABEL:
            fprintf(gOutFile, "%s:array_data_%x\n", sp.c_str(), (u2)insnIdx);
            break;
 ...
 */
static const char *strBool(bool val) { return val ? "true" : "false"; }
...
//             }
//             // No good option so just use java encoding, too many chars are
//             // invalid
 ...
 * Dumps encoded value.
 */
static void dumpEncodedValue(const DexFile *pDexFile,
                             const u1 **data);  // forward
static void dumpEncodedValue(const DexFile *pDexFile, const u1 **data, u1 type,
                             u1 arg) {
    switch (type) {
        case DexFile::kDexAnnotationByte:
            fprintf(gOutFile, "%" PRId8,
                    static_cast<s1>(readVarWidth(data, arg, false)));
            break;
        case DexFile::kDexAnnotationShort:
            fprintf(gOutFile, "%" PRId16,
                    static_cast<s2>(readVarWidth(data, arg, true)));
            break;
        case DexFile::kDexAnnotationChar:
            fprintf(gOutFile, "%" PRIu16,
                    static_cast<u2>(readVarWidth(data, arg, false)));
            break;
        case DexFile::kDexAnnotationInt:
            fprintf(gOutFile, "%" PRId32,
                    static_cast<s4>(readVarWidth(data, arg, true)));
            break;
        case DexFile::kDexAnnotationLong:
            fprintf(gOutFile, "%" PRId64,
                    static_cast<s8>(readVarWidth(data, arg, true)));
            break;
        case DexFile::kDexAnnotationFloat: {
            ...
            conv.data = static_cast<u4>(readVarWidth(data, arg, false))
                        << (3 - arg) * 8;
        ...
        case DexFile::kDexAnnotationEnum: {
            const u4 field_idx =
                static_cast<u4>(readVarWidth(data, arg, false));
        ...
        case DexFile::kDexAnnotationAnnotation: {
            ...
            fputs(pDexFile->StringByTypeIdx(dex::TypeIndex(type_idx)),
                  gOutFile);
            ...
            for (u4 i = 0; i < size; i++) {
                ...
                fputc(' ', gOutFile);
                fputs(pDexFile->StringDataByIdx(dex::StringIndex(name_idx)),
                      gOutFile);
                fputc('=', gOutFile);
                dumpEncodedValue(pDexFile, data);
            }
...
//     pHeader.string_ids_size_); fprintf(gOutFile, "string_ids_off      : %d
//     (0x%06x)\n",
...
//     pHeader.method_ids_size_); fprintf(gOutFile, "method_ids_off      : %d
//     (0x%06x)\n",
//             pHeader.method_ids_off_, pHeader.method_ids_off_);
//     fprintf(gOutFile, "class_defs_size     : %d\n",
//     pHeader.class_defs_size_); fprintf(gOutFile, "class_defs_off      : %d
//     (0x%06x)\n",
...
//     accessor.NumStaticFields()); fprintf(gOutFile, "instance_fields_size:
//     %d\n",
 ...
 */
static void dumpAnnotationSetItem(const DexFile *pDexFile,
                                  const dex::AnnotationSetItem *set_item) {
    ...
    for (u4 i = 0; i < set_item->size_; i++) {
        const dex::AnnotationItem *annotation =
            pDexFile->GetAnnotationItem(set_item, i);
        ...
        switch (annotation->visibility_) {
            case DexFile::kDexVisibilityBuild:
                fputs("VISIBILITY_BUILD ", gOutFile);
                break;
            case DexFile::kDexVisibilityRuntime:
                fputs("VISIBILITY_RUNTIME ", gOutFile);
                break;
            case DexFile::kDexVisibilitySystem:
                fputs("VISIBILITY_SYSTEM ", gOutFile);
                break;
            default:
                fputs("VISIBILITY_UNKNOWN ", gOutFile);
                break;
        }  // switch
        // Decode raw bytes in annotation.
        const u1 *rData = annotation->annotation_;
        dumpEncodedValue(pDexFile, &rData, DexFile::kDexAnnotationAnnotation,
                         0);
        fputc('\n', gOutFile);
    }
}
...
static void dumpClassAnnotations(const DexFile *pDexFile, int idx) {
    const dex::ClassDef &pClassDef = pDexFile->GetClassDef(idx);
    const dex::AnnotationsDirectoryItem *dir =
        pDexFile->GetAnnotationsDirectory(pClassDef);
    if (dir == nullptr) {
        return;  // none
    }
    ...
    const dex::AnnotationSetItem *class_set_item =
        pDexFile->GetClassAnnotationSet(dir);
    const dex::FieldAnnotationsItem *fields =
        pDexFile->GetFieldAnnotations(dir);
    const dex::MethodAnnotationsItem *methods =
        pDexFile->GetMethodAnnotations(dir);
    const dex::ParameterAnnotationsItem *pars =
        pDexFile->GetParameterAnnotations(dir);
    ...
    if (fields != nullptr) {
        for (u4 i = 0; i < dir->fields_size_; i++) {
            ...
            const char *field_name =
                pDexFile->StringDataByIdx(pFieldId.name_idx_);
            fprintf(gOutFile, "Annotations on field #%u '%s'\n", field_idx,
                    field_name);
            dumpAnnotationSetItem(
                pDexFile, pDexFile->GetFieldAnnotationSetItem(fields[i]));
        }
    }
    ...
    if (methods != nullptr) {
        for (u4 i = 0; i < dir->methods_size_; i++) {
            ...
            const char *method_name =
                pDexFile->StringDataByIdx(pMethodId.name_idx_);
            fprintf(gOutFile, "Annotations on method #%u '%s'\n", method_idx,
                    method_name);
            dumpAnnotationSetItem(
                pDexFile, pDexFile->GetMethodAnnotationSetItem(methods[i]));
        }
    }
    ...
    if (pars != nullptr) {
        for (u4 i = 0; i < dir->parameters_size_; i++) {
            ...
            const dex::AnnotationSetRefList *list =
                pDexFile->GetParameterAnnotationSetRefList(&pars[i]);
            if (list != nullptr) {
                for (u4 j = 0; j < list->size_; j++) {
                    fprintf(gOutFile, "#%u\n", j);
                    dumpAnnotationSetItem(
                        pDexFile, pDexFile->GetSetRefItemItem(&list->list_[j]));
                }
            }
        }
    }
 ...
 */
static void dumpInterface(const DexFile *pDexFile,
                          const dex::TypeItem &pTypeItem) {
    const char *interfaceName = pDexFile->StringByTypeIdx(pTypeItem.type_idx_);

    fprintf(gOutFile, ".implements %s\n", interfaceName);
}

/*
 * Dumps the catches table associated with the code.
 */
static std::vector<TryCatch> dumpCatches(const DexFile *pDexFile,
    ...
    // No catch table.
    if (triesSize == 0) {
        std::vector<TryCatch> m;
        return m;
    }

    std::vector<TryCatch> tcs;
    ...
    for (const dex::TryItem &try_item : accessor.TryItems()) {
        ...
        std::map<u4, std::string> catchhandles;
        for (CatchHandlerIterator it(accessor, try_item); it.HasNext();
             it.Next()) {
            const dex::TypeIndex tidx = it.GetHandlerTypeIndex();
            const char *descriptor =
                (!tidx.IsValid()) ? "<all>" : pDexFile->StringByTypeIdx(tidx);
            catchhandles.insert_or_assign(it.GetHandlerAddress(), descriptor);

        }  // for

        tc.catches = catchhandles;
...
static std::unique_ptr<char[]> indexString(const DexFile *pDexFile,
    ...
    switch (Instruction::IndexTypeOf(pDecInsn->Opcode())) {
        ...
        case Instruction::kIndexTypeRef:
            if (index < pDexFile->GetHeader().type_ids_size_) {
                const char *tp =
                    pDexFile->StringByTypeIdx(dex::TypeIndex(index));
                outSize = snprintf(buf.get(), bufSize, "%s ", tp);
            } else {
                outSize = snprintf(buf.get(), bufSize, "<type?> ");
            }
            break;
        case Instruction::kIndexStringRef:
            if (index < pDexFile->GetHeader().string_ids_size_) {
                const char *st =
                    pDexFile->StringDataByIdx(dex::StringIndex(index));
                outSize = snprintf(buf.get(), bufSize, "\"%s\"", st);
            } else {
                outSize = snprintf(buf.get(), bufSize, "<string?> ");
            }
            break;
        case Instruction::kIndexMethodRef:
            if (index < pDexFile->GetHeader().method_ids_size_) {
                ...
                const Signature signature =
                    pDexFile->GetMethodSignature(pMethodId);
                const char *backDescriptor =
                    pDexFile->StringByTypeIdx(pMethodId.class_idx_);
                outSize =
                    snprintf(buf.get(), bufSize, "%s->%s%s", backDescriptor,
                             name, signature.ToString().c_str());
            } else {
                outSize = snprintf(buf.get(), bufSize, "<method?> ");
            }
            break;
        case Instruction::kIndexFieldRef:
            if (index < pDexFile->GetHeader().field_ids_size_) {
                ...
                const char *typeDescriptor =
                    pDexFile->StringByTypeIdx(pFieldId.type_idx_);
                const char *backDescriptor =
                    pDexFile->StringByTypeIdx(pFieldId.class_idx_);
                outSize = snprintf(buf.get(), bufSize, "%s->%s:%s ",
                                   backDescriptor, name, typeDescriptor);
            } else {
                outSize = snprintf(buf.get(), bufSize, "<field?> ");
            }
            break;
        ...
        case Instruction::kIndexMethodAndProtoRef: {
            ...
            if (index < pDexFile->GetHeader().method_ids_size_) {
                ...
                const char *backDescriptor =
                    pDexFile->StringByTypeIdx(pMethodId.class_idx_);
                method = android::base::StringPrintf(
                    "%s.%s:%s", backDescriptor, name,
                    signature.ToString().c_str());
            }
            if (secondary_index < pDexFile->GetHeader().proto_ids_size_) {
                const dex::ProtoId &protoId =
                    pDexFile->GetProtoId(dex::ProtoIndex(secondary_index));
                const Signature signature =
                    pDexFile->GetProtoSignature(protoId);
                proto = signature.ToString();
            }
            outSize =
                snprintf(buf.get(), bufSize,
                         "%s, %s // method@%0*x, proto@%0*x", method.c_str(),
                         proto.c_str(), width, index, width, secondary_index);
            break;
        }
        case Instruction::kIndexCallSiteRef:
            // Call site information is too large to detail in disassembly so
            // just output the index.
            outSize =
                snprintf(buf.get(), bufSize, "call_site@%0*x", width, index);
            break;
        case Instruction::kIndexMethodHandleRef:
            // Method handle information is too large to detail in disassembly
            // so just output the index.
            outSize = snprintf(buf.get(), bufSize, "method_handle@%0*x", width,
                               index);
            break;
        case Instruction::kIndexProtoRef:
            if (index < pDexFile->GetHeader().proto_ids_size_) {
                const dex::ProtoId &protoId =
                    pDexFile->GetProtoId(dex::ProtoIndex(index));
                const Signature signature =
                    pDexFile->GetProtoSignature(protoId);
                const std::string &proto = signature.ToString();
                outSize = snprintf(buf.get(), bufSize, "%s // proto@%0*x",
                                   proto.c_str(), width, index);
            } else {
                outSize = snprintf(buf.get(), bufSize, "<?> // proto@%0*x",
                                   width, index);
            }
            break;
    }  // switch
    ...
    if (outSize >= bufSize) {
        // The buffer wasn't big enough; retry with computed size. Note:
        // snprintf() doesn't count/ the '\0' as part of its returned size, so
        // we add explicit space for it here.
        return indexString(pDexFile, pDecInsn, outSize + 1);
    }
    return buf;
}

inline void printSwitch(std::map<u4, std::map<u4, u4>> *switch_data,
                        u4 insIdx) {
    auto case_ = switch_data->find(insIdx);
    if (case_ != switch_data->end()) {
        for (auto pair_ : (*case_).second) {
            fprintf(gOutFile, "        0x%x -> 0x%x\n", pair_.first,
                    pair_.second);
        }
    }
}
 ...
 */
static void dumpInstruction(const DexFile *pDexFile, const dex::CodeItem *pCode,
                            u4 insnIdx, const Instruction *pDecInsn, int pnum, std::map<u4, std::map<u4, u4>>* switch_data) {
    ...
    //             const u1 *bytePtr = (const u1 *)&accessor.Insns()[insnIdx +
    //             i]; fprintf(gOutFile, " %02x%02x", bytePtr[0], bytePtr[1]);
    //         }
    //     } else {
    //         fputs("     ", gOutFile);
    //     }
    ...
    if (pDecInsn->Opcode() == Instruction::NOP) {
        ...
        if (instr == Instruction::kPackedSwitchSignature) {
            fprintf(gOutFile, "    .packed-switch \n");
            printSwitch(switch_data, insnIdx);
            fprintf(gOutFile, "    .end-packed-switch \n");
        }
        else if (instr == Instruction::kSparseSwitchSignature) {
            fprintf(gOutFile, "    .sparse-switch\n");
            printSwitch(switch_data, insnIdx);
            fprintf(gOutFile, "    .end-sparse-switch \n");
        } else if (instr == Instruction::kArrayDataSignature) {
            u1 *insstart = (u1 *)&accessor.Insns()[insnIdx];
            u2 elem_size = get2LE(insstart + 2);
            u4 array_size = *(int *)(insstart + 4);
            fprintf(gOutFile, "    .array-data %hi (%u units)\n", elem_size, array_size);
            fprintf(gOutFile, "    .end-array-data\n");
        } else {
            fprintf(gOutFile, "    nop // spacer");
        }
    } else {
        fprintf(gOutFile, "    %s", pDecInsn->Name());
    }

    // Set up additional argument.
    std::unique_ptr<char[]> indexBuf;
    if (Instruction::IndexTypeOf(pDecInsn->Opcode()) !=
        Instruction::kIndexNone) {
    ...
    switch (Instruction::FormatOf(pDecInsn->Opcode())) {
        ...
        case Instruction::k12x: {  // op vA, vB
            cvA = getporv(pDecInsn->VRegA(), pnum, &vA);
            cvB = getporv(pDecInsn->VRegB(), pnum, &vB);
            fprintf(gOutFile, " %c%d, %c%d", cvA, vA, cvB, vB);
            break;
        }
        case Instruction::k11n: {  // op vA, #+B
            cvA = getporv(pDecInsn->VRegA(), pnum, &vA);
            fprintf(gOutFile, " %c%d, 0x%x // #int %d", cvA, vA,
                    pDecInsn->VRegB(), pDecInsn->VRegB());
            break;
        }
        case Instruction::k11x: {  // op vAA
            cvA = getporv(pDecInsn->VRegA(), pnum, &vA);
            fprintf(gOutFile, " %c%d", cvA, vA);
            break;
        }
        ...
        case Instruction::k20t: {  // op +AAAA
            ...
            fprintf(gOutFile, " :goto_%x ", insnIdx + targ);
            break;
        }
        ...
        case Instruction::k21t: {  // op vAA, +BBBB
            ...
            fprintf(gOutFile, " %c%d, :cond_%x", cvA, vA, insnIdx + targ);
            break;
        }
        ...
        case Instruction::k21h:  // op vAA, #+BBBB0000[00000000]
            ...
            if (pDecInsn->Opcode() == Instruction::CONST_HIGH16) {
                ...
                fprintf(gOutFile, " %c%d, 0x%x // #long %" PRId64, cvA, vA,
                        pDecInsn->VRegB(), (long)pDecInsn->VRegB());
            }
            break;
        ...
        case Instruction::k31c: {  // op vAA, thing@BBBBBBBB
            ...
            fprintf(gOutFile, " %c%d, %s", cvA, vA, indexBuf.get());
            break;
        }
        case Instruction::k23x: {  // op vAA, vBB, vCC
            cvA = getporv(pDecInsn->VRegA(), pnum, &vA);
            cvB = getporv(pDecInsn->VRegB(), pnum, &vB);
            cvC = getporv(pDecInsn->VRegB(), pnum, &vC);
            fprintf(gOutFile, " %c%d, %c%d, %c%d", cvA, vA, cvB, vB, cvC, vC);
            break;
        }
        case Instruction::k22b: {  // op vAA, vBB, #+CC
            cvA = getporv(pDecInsn->VRegA(), pnum, &vA);
            cvB = getporv(pDecInsn->VRegB(), pnum, &vB);
            fprintf(gOutFile, " %c%d, %c%d, 0x%x // #int %d", cvA, vA, cvB, vB,
                    pDecInsn->VRegC(), pDecInsn->VRegC());
            break;
        }
        case Instruction::k22t: {  // op vA, vB, +CCCC
            ...
            fprintf(gOutFile, " %c%d, %c%d, :cond_%x", cvA, vA, cvB, vB,
                    insnIdx + targ);
            break;
        }
        ...
        case Instruction::k22c: {  // op vA, vB, thing@CCCC
                                   // NOT SUPPORTED:
                                   // case Instruction::k22cs:    // [opt] op
                                   // vA, vB, field offset CCCC
            cvA = getporv(pDecInsn->VRegA(), pnum, &vA);
            cvB = getporv(pDecInsn->VRegB(), pnum, &vB);
            fprintf(gOutFile, " %c%d, %c%d, %s", cvA, vA, cvB, vB,
                    indexBuf.get());
            break;
        }
        ...
        case Instruction::k31i: {  // op vAA, #+BBBBBBBB
            ...
            fprintf(gOutFile, " %c%d, 0x%x // #float %g", cvA, vA,
                    pDecInsn->VRegB(), conv.f);
            break;
        }
        case Instruction::k31t: {  // op vAA, offset +BBBBBBBB
            cvA = getporv(pDecInsn->VRegA(), pnum, &vA);
            u4 idx = insnIdx + pDecInsn->VRegB();
            switch (pDecInsn->Opcode()) {
                case Instruction::SPARSE_SWITCH: {
                    fprintf(gOutFile, " %c%d, :sswitch_%x", cvA, vA, idx);
                } break;
                case Instruction::PACKED_SWITCH: {
                    fprintf(gOutFile, " %c%d, :sswitch_%x", cvA, vA, idx);
                } break;
                case Instruction::FILL_ARRAY_DATA: {
                    fprintf(gOutFile, " %c%d, :array_data_%x", cvA, vA, idx);
                } break;
        ...
        case Instruction::k45cc: {  // op {vC, vD, vE, vF, vG}, method@BBBB,
                                    // proto@HHHH
                                    // NOT SUPPORTED:
        ...
        case Instruction::k4rcc: {  // op {vCCCC .. v(CCCC+AA-1)}, method@BBBB,
                                    // proto@HHHH NOT SUPPORTED:
            // case Instruction::k3rms:       // [opt]
            // invoke-virtual+super/range case Instruction::k3rmi:       //
            // [opt] execute-inline/range This doesn't match the "dx" output
            // when some of the args are 64-bit values -- dx only shows the
            // first register.

            fputs(" {", gOutFile);
            for (int i = 0, n = pDecInsn->VRegA(); i < n; i++) {
                cvC = getporv(pDecInsn->VRegC() + i, pnum, &vC);
                if (i == 0) {
                    fprintf(gOutFile, "%c%d", cvC, vC);
                } else {
                    fprintf(gOutFile, ", %c%d", cvC, vC);
                }
            }  // for
        ...
        case Instruction::k51l: {  // op vAA, #+BBBBBBBBBBBBBBBB
            ...
            fprintf(gOutFile, " %c%d, %lx // #double %g", cvA, vA,
                    pDecInsn->WideVRegB(), conv.d);
            break;
        }
        // NOT SUPPORTED:
...
static void dumpBytecodes(const DexFile *pDexFile, u4 idx,
                          const dex::CodeItem *pCode,
                          std::map<int32_t, int> lines, int pnum,
    ...
    // fprintf(gOutFile,
    //         "%06x:                                        |[%06x]
    //         %s.%s:%s\n", codeOffset, codeOffset, dot.get(), name,
    //         signature.ToString().c_str());

    // Iterate over all instructiidxons.
    auto cmp = [](LabelType a, LabelType b) {
        if (a.idx != b.idx) {
            return a.idx < b.idx;
        }
        return static_cast<int>(a.ltype) < static_cast<int>(b.ltype);
    };

    std::set<LabelType, decltype(cmp)> jumplabels(cmp);

    CodeItemDataAccessor accessor(*pDexFile, pCode);

    std::map<u4, bool> type_sw;

    std::map<u4, std::map<u4, u4>> switch_data;

    u4 maxPc = accessor.InsnsSizeInCodeUnits();
    for (auto pair = accessor.begin(); pair != accessor.end(); pair++) {
        ...
        switch (Instruction::FormatOf(instruction->Opcode())) {
            ...
            case Instruction::k20t: {  // op +AAAA
                ...
                ltype.idx = dexPc + targ;
                ltype.ltype = LabelEnum::GOTO_LABEL;
                break;
            }
            case Instruction::k21t: {  // op vAA, +BBBB
                const s4 targ = (s4)instruction->VRegB();
                ltype.idx = dexPc + targ;
                ltype.ltype = LabelEnum::IF_LABEL;
                break;
            }
            case Instruction::k22t: {  // op vA, vB, +CCCC
            ...
            case Instruction::k31t: {  // op vAA, offset +BBBBBBBB
                ltype.idx = dexPc + instruction->VRegB();

                u1 *insstartp = (u1 *)&accessor.Insns()[ltype.idx];

                const u2 instr = get2LE(insstartp);
                if (instr == Instruction::kSparseSwitchSignature) {
                    ltype.ltype = LabelEnum::DATA_SPARSE_SWITCH_LABEL;
                    u2 size_switch = get2LE(insstartp + 2);
                    fprintf(gOutFile, "    sparse-switch-data (%d units)\n",
                            size_switch);
                    int* keys = (int *)(insstartp + 4);
                    int* offsets = &(keys[size_switch]);
                    std::map<u4, u4> cases;
                    for(int i = 0; i < (int)size_switch; i++){
                        cases.insert_or_assign(get4LE((unsigned char *)&keys[i]), get4LE((unsigned char *)&offsets[i]));
                        LabelType sltype;
                        sltype.idx = get4LE((unsigned char *)&offsets[i]);
                        sltype.ltype = LabelEnum::SPARSE_SWITCH_LABEL;
                        jumplabels.insert(sltype);
                    }
                    switch_data.insert_or_assign(ltype.idx, cases);
                    type_sw.insert_or_assign(ltype.idx, true);
                } else if (instr == Instruction::kPackedSwitchSignature) {
                    ltype.ltype = LabelEnum::DATA_PACKED_SWITCH_LABEL;
                    u2 size_switch = get2LE(insstartp + 2);
                    int first_key = *(int *)(insstartp + 4);
                    
                    int* offsets = (int *)(insstartp + 8);
                    std::map<u4, u4> cases;
                    for(int i = 0; i < (int)size_switch; i++){
                        cases.insert_or_assign(first_key+i, get4LE((unsigned char *)&offsets[i]));
                        LabelType sltype;
                        sltype.idx = get4LE((unsigned char *)&offsets[i]);
                        sltype.ltype = LabelEnum::PACKED_SWITCH_LABEL;
                        jumplabels.insert(sltype);
                    }
                    switch_data.insert_or_assign(ltype.idx, cases);
                    type_sw.insert_or_assign(ltype.idx, false);
                } else if (instr == Instruction::kArrayDataSignature) {
                    ltype.ltype = LabelEnum::ARRAY_DATA_LABEL;
                    u2 elem_size = get2LE(insstartp + 2);
                    u4 array_size = *(int *)(insstartp + 4);
                    fprintf(gOutFile, "    array-data %hi (%u units)\n", elem_size, array_size);
                }
                break;
            }
    ...
    for (auto pair = accessor.begin(); pair != accessor.end(); pair++) {
        const u4 dexPc = pair.DexPc();
        if (dexPc >= maxPc) {
            LOG(WARNING) << "GLITCH: run-away instruction at idx=0x" << std::hex
                         << dexPc;
            break;
        }
        const Instruction *instruction = &pair.Inst();
        const u4 insnWidth = instruction->SizeInCodeUnits();
        if (insnWidth == 0) {
            LOG(WARNING) << "GLITCH: zero-width instruction at idx=0x"
                         << std::hex << dexPc;
            break;
        }
        ...
        for (size_t i = 0; i < trycatches.size(); i++) {
            TryCatch tc = *(trycatches.begin() + i);
            if (tc.end == dexPc) {
                printLabel(LabelEnum::TRY_END_LABEL, dexPc);
                EndIdx = i;
                break;
            }
        }
        ...
        auto line = lines.find(dexPc);
        if (line != lines.end()) {
            fprintf(gOutFile, "    .line %d\n", (*line).second);
            // fprintf(gOutFile, "    .line %d\n", (*line).second);
        }

        for (LabelType lt : jumplabels) {
            if (lt.idx == dexPc) {
                printLabel(lt.ltype, dexPc);
            }
        }
        ...
        if (EndIdx == -1) {
            for (TryCatch tc : trycatches) {
                if (tc.start == dexPc) {
                    printLabel(LabelEnum::TRY_START_LABEL, dexPc);
                    break;
                }
            }
        } else {
            TryCatch tc = *(trycatches.begin() + EndIdx);
            if (tc.end == dexPc) {
                for(auto catch_ : tc.catches){
                    printcatch(tc.start, tc.end, catch_);
                }
            }
        }


        dumpInstruction(pDexFile, pCode, dexPc, instruction, pnum, &switch_data);
    }  // for
}
 ...
 */
static void dumpCode(const DexFile *pDexFile, u4 idx,
                     const dex::CodeItem *pCode) {
    ...
    // Try-catch blocks.
    auto trycatch = dumpCatches(pDexFile, pCode);

    dumpBytecodes(pDexFile, idx, pCode, lines,
                  accessor.RegistersSize() - accessor.InsSize(), trycatch);

    fprintf(gOutFile, ".end method\n");
}
...
static std::string GetHiddenapiFlagStr(uint32_t hiddenapi_flags) {
    ...
    std::transform(str_api_list.begin(), str_api_list.end(),
                   str_api_list.begin(), ::toupper);
    return str_api_list;
}
 ...
 * Dumps a method.
 */
static void dumpMethod(const ClassAccessor::Method &method) {
    ...
    if (method.GetCodeItem() != nullptr) {
        dumpCode(&dex_file, method.GetIndex(), method.GetCodeItem());
    }
 ...
 */
static void dumpField(const ClassAccessor::Field &field,
                      const u1 **data = nullptr) {
    // Bail for anything private if export only requested.
 ...
 */
static void dumpClass(const DexFile *pDexFile, int idx) {
    const dex::ClassDef &pClassDef = pDexFile->GetClassDef(idx);

    // dumpClassDef(pDexFile);
    ...
    const char *classDescriptor =
        pDexFile->StringByTypeIdx(pClassDef.class_idx_);
    ...
    // General class information.
    char *accessStr =
        createAccessFlagStr(pClassDef.access_flags_, kAccessForClass);
    ...
    if (pInterfaces != nullptr) {
        ...
        for (u4 i = 0; i < pInterfaces->Size(); i++) {
            dumpInterface(pDexFile, pInterfaces->GetTypeItem(i));
        }  // for
        fprintf(gOutFile, "\n\n");
    }
    ...
    uint32_t i = 0u;
    auto sfield = accessor.GetStaticFields();
    if (sfield.begin() != sfield.end()) {
        fprintf(gOutFile, "# static fields\n");
        for (const ClassAccessor::Field &field : sfield) {
            dumpField(field, i < sSize ? &sData : nullptr);
            ++i;
            fprintf(gOutFile, "\n");
        }
        fprintf(gOutFile, "\n");
    ...
    // Instance fields.
    auto ifield = accessor.GetInstanceFields();
    if (ifield.begin() != ifield.end()) {
        fprintf(gOutFile, "# instance fields\n");
        i = 0u;
        for (const ClassAccessor::Field &field : ifield) {
            dumpField(field);
            ++i;
            fprintf(gOutFile, "\n");
        }
    ...
    auto dmethods = accessor.GetDirectMethods();

    if (dmethods.begin() != dmethods.end()) {
        fprintf(gOutFile, "# direct methods\n");
        i = 0u;
        for (const ClassAccessor::Method &method : dmethods) {
            dumpMethod(method);
            fprintf(gOutFile, "\n");
            ++i;
        }
    ...
    auto vmethods = accessor.GetVirtualMethods();

    if (vmethods.begin() != vmethods.end()) {
        fprintf(gOutFile, "# virtual methods\n");
        i = 0u;
        for (const ClassAccessor::Method &method : vmethods) {
            dumpMethod(method);
            fprintf(gOutFile, "\n");
            ++i;
        }
    ...
    if (type != nullptr) {
        if (is_invoke) {
            const dex::MethodId &method_id =
                pDexFile->GetMethodId(mh.field_or_method_idx_);
            declaring_class =
                pDexFile->GetMethodDeclaringClassDescriptor(method_id);
            ...
            const dex::FieldId &field_id =
                pDexFile->GetFieldId(mh.field_or_method_idx_);
            declaring_class =
                pDexFile->GetFieldDeclaringClassDescriptor(field_id);
            member = pDexFile->GetFieldName(field_id);
            member_type = pDexFile->GetFieldTypeDescriptor(field_id);
        }
        if (is_instance) {
            member_type = android::base::StringPrintf("(%s%s", declaring_class,
                                                      member_type.c_str() + 1);
        }
    } else {
        type = "?";
...
static void dumpCallSite(const DexFile *pDexFile, u4 idx) {
    ...
    it.Next();
    dex::StringIndex method_name_idx =
        static_cast<dex::StringIndex>(it.GetJavaValue().i);
    ...
    std::string method_type =
        pDexFile->GetProtoSignature(method_type_id).ToString();
    it.Next();

    fprintf(gOutFile, "Call site #%u: // offset %u\n", idx,
            call_site_id.data_off_);
    ...
    while (it.HasNext()) {
        ...
        switch (it.GetValueType()) {
            ...
            case EncodedArrayValueIterator::ValueType::kLong:
                type = "long";
                value = android::base::StringPrintf("%" PRId64,
                                                    it.GetJavaValue().j);
                break;
            ...
            case EncodedArrayValueIterator::ValueType::kType: {
                ...
                dex::TypeIndex type_idx =
                    static_cast<dex::TypeIndex>(it.GetJavaValue().i);
            ...
            case EncodedArrayValueIterator::ValueType::kAnnotation:
                // Unreachable based on current
                // EncodedArrayValueIterator::Next().
 ...
 */
static void processDexFile(const DexFile *pDexFile) {
    // Headers.
...
int processFile(const char *fileName) {
    ...
    if (!dex_file_loader.OpenAll(
            ...
            fileName, false, false, &error_code, &error_msg, &dex_files)) {
    ...
    for (size_t i = 0, n = dex_files.size(); i < n; i++) {
        processDexFile(dex_files[i].get());
    }
